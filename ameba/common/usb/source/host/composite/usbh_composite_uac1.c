...
#include "usb_uac1.h"
#if CONFIG_REALTEK_AMEBA_ZEPHYR_USB
...
#if CONFIG_REALTEK_AMEBA_ZEPHYR_USB
static int uach_comp_mute(const struct shell *sh, size_t argc,
						  char *argv[])
{
	int status = HAL_OK;
	u8 mute;
...
static int uach_comp_vol(const struct shell *sh, size_t argc,
						 char *argv[])
{
	usbh_composite_uac_t *uac = &usbh_composite_uac;
	int status = HAL_OK;
...
static int uach_comp_volup(const struct shell *sh, size_t argc,
						   char *argv[])
{
	UNUSED(argc);
	UNUSED(argv);
...
static int uach_comp_voldown(const struct shell *sh, size_t argc,
							 char *argv[])
{
	UNUSED(argc);
	UNUSED(argv);
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_usbh_cmds,
							   SHELL_CMD_ARG(mute, NULL,
									   "uach_comp mute <1/0>",
									   uach_comp_mute, 2, 0),
							   SHELL_CMD_ARG(vol, NULL,
									   "uach_comp vol <vol>",
									   uach_comp_vol, 2, 0),
							   SHELL_CMD_ARG(volup, NULL,
									   "uach_comp volup",
									   uach_comp_volup, 1, 0),
							   SHELL_CMD_ARG(voldown, NULL,
									   "uach_comp voldown",
									   uach_comp_voldown, 1, 0),
							   SHELL_SUBCMD_SET_END
							  );

SHELL_CMD_REGISTER(uach_comp, &sub_usbh_cmds, "USBH Composite UAC commands", NULL);
#else
