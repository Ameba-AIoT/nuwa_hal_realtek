...
#if CONFIG_REALTEK_AMEBA_ZEPHYR_USB
static int uach_mute(const struct shell *sh, size_t argc,
					 char *argv[])
{
	int status = HAL_OK;
	u8 mute;
...
static int uach_vol(const struct shell *sh, size_t argc,
					char *argv[])
{
	usbh_uac_host_t *uac = &usbh_uac_host;
	int status = HAL_OK;
...
static int uach_volup(const struct shell *sh, size_t argc,
					  char *argv[])
{
	UNUSED(argc);
	UNUSED(argv);
...
static int uach_voldown(const struct shell *sh, size_t argc,
						char *argv[])
{
	UNUSED(argc);
	UNUSED(argv);
...
SHELL_CMD_REGISTER(uach, &sub_usbh_cmds, "USB Audio Host commands", NULL);
#else
