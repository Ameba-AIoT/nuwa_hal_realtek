...
#include "ameba_sd.h"
#endif
#if !CONFIG_COMP_MSC_RAM_DISK && !CONFIG_COMP_MSC_RAM_DISK_ZEPHYR_FATFS
#include "vfs_fatfs.h"
#elif CONFIG_COMP_MSC_RAM_DISK_ZEPHYR_FATFS
...
static int RAM_init(void)
{
	...
	STRUCT_SECTION_FOREACH(usbd_composite_msc_lun, lun) {
		mdev->disk = lun->disk;
		ret = HAL_OK;
	}
...
static int RAM_GetCapacity(u32 *sector_count)
{
	...
	if (disk_access_ioctl(mdev->disk, DISK_IOCTL_GET_SECTOR_COUNT, sector_count) != 0) {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "Get capacity fail: %d\n", *sector_count);
		*sector_count = 0;
		return HAL_ERR_MEM;
	}
...
static int RAM_ReadBlocks(u32 sector, u8 *data, u32 count)
{
	...
	if (disk_access_read(mdev->disk, data, sector, count) == 0) {
		ret = HAL_OK;
	}
...
static int RAM_WriteBlocks(u32 sector, const u8 *data, u32 count)
{
	...
	if (disk_access_write(mdev->disk, data, sector, count) == 0) {
		ret = HAL_OK;
	}
