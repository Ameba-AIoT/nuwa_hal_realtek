...
#if !CONFIG_USBD_MSC_RAM_DISK
#include "ameba_sd.h"
#include "vfs_fatfs.h"
#elif CONFIG_USBD_MSC_RAM_DISK_ZEPHYR_FATFS
...
static int RAM_GetCapacity(u32 *sector_count)
{
	...
	if (disk_access_ioctl(cdev->disk, DISK_IOCTL_GET_SECTOR_COUNT, sector_count) != 0) {
...
static int RAM_ReadBlocks(u32 sector, u8 *data, u32 count)
{
	...
	if (disk_access_read(cdev->disk, data, sector, count) == 0) {
		ret = HAL_OK;
	}
...
static int RAM_WriteBlocks(u32 sector, const u8 *data, u32 count)
{
	...
	if (disk_access_write(cdev->disk, data, sector, count) == 0) {
		ret = HAL_OK;
	}
