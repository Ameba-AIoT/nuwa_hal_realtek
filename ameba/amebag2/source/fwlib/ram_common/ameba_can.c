...
void CAN_Init(CAN_TypeDef *CANx, CAN_InitTypeDef *CAN_InitStruct)
{
	...
	can_bit_timing |= CAN_BRP(CAN_InitStruct->CAN_BitPrescaler - 1) |
					  CAN_SJW(CAN_InitStruct->CAN_SJW) |
					  CAN_TSEG2(CAN_InitStruct->CAN_TSEG2 - 1) |
					  CAN_TSEG1(CAN_InitStruct->CAN_TSEG1 - 1);
...
/**
  ...
  * @retval None
  */
void CAN_WakeFilterConfig(CAN_TypeDef *CANx, u32 FltNum)
{
	assert_param(IS_CAN_ALL_PERIPH(CANx));
...
void CAN_SetRxMsgBuf(CAN_TypeDef *CANx, CAN_RxMsgTypeDef *RxMsg)
{
	...
	if (RxMsg->IDE == CAN_STANDARD_FRAME) {
		CANx->CAN_RAM_MASK = CAN_RAM_ID_MASK(RxMsg->ID_MASK << 18);
	} else {
		CANx->CAN_RAM_MASK = CAN_RAM_ID_MASK(RxMsg->ID_MASK);
	}
...
void CAN_TxAutoReply(CAN_TypeDef *CANx, CAN_TxMsgTypeDef *TxMsg)
{
	...
	if (TxMsg->RTR == CAN_DATA_FRAME) {
		for (i = 0; i < 16; i++) {
			CANx->CAN_RAM_DATA_x[i] = TxMsg->Data[(16 - i - 1) * 4];
		}
	}
...
/**
  ...
  * @retval DMA data
  */
void CAN_RamBufferMapConfig(CAN_TypeDef *CANx, u32 *pPara)
{
	u32 i;

	/*message buffer address*/
	for (i = 0; i < CAN_MESSAGE_BUFFER_SIZE; i++) {
		CAN_MsgBaseAddrConfig(CANx, i, *(pPara + i));
	}

	/*end address*/
	CAN_MsgBaseAddrEndReg(CANx, *(pPara + CAN_MESSAGE_BUFFER_SIZE));
}
