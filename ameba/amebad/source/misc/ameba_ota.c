...
static void ota_update_local_task(void *param)
{
	...
	if (download_new_fw_from_server(server_socket, &OtaTargetHdr, ota_target_index) == FALSE) {
		goto update_ota_exit;
	}
...
restart_http_ota:
	...
	/*---------step5: download new firmware from server and write it to flash--------*/
	if (download_new_fw_from_server_http(alloc, writelen, server_socket, &OtaTargetHdr, ota_target_index) == FALSE) {
		goto update_ota_exit;
	}
...
restart_https_ota:
	...
	/*---------step5: download new firmware from server and write it to flash--------*/
	if (download_new_fw_from_server_https(alloc, writelen, &ssl, &OtaTargetHdr, ota_target_index) == FALSE) {
		goto update_ota_exit;
	}
