...
int rtl_crypto_hmac_sha1_init(IN const u8 *key, IN const u32 keylen)
{
	...
	if (key == NULL) {
		if (hmac_sha1_key == NULL) {
			return _ERRNO_CRYPTO_NULL_POINTER;
		}
		if ((u32)(hmac_sha1_key) & 0x3) {
			return _ERRNO_CRYPTO_ADDR_NOT_4Byte_Aligned;    // need to be 4 byte alignment
		}
		if (hmac_sha1_keylen > CRYPTO_AUTH_PADDING) {
	...
	IN const u8 *key, 	IN const u32 keylen)
{
	...
	if (key == NULL) {
		if (hmac_sha2_key == NULL) {
			return _ERRNO_CRYPTO_NULL_POINTER;
		}
		if ((u32)(hmac_sha2_key) & 0x3) {
			return _ERRNO_CRYPTO_ADDR_NOT_4Byte_Aligned;    // need to be 4 byte alignment
		}
		if (hmac_sha2_keylen > CRYPTO_AUTH_PADDING) {
			return _ERRNO_CRYPTO_KEY_OutRange;
		}
