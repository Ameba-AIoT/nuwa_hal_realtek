...
static const char *const TAG = "CRYPTO";
...
void CRYPTO_RegDump(void)
{
	...
	RTK_LOGI(TAG, "=========================================\n");
	RTK_LOGI(TAG, "Crypto Engine Registers\n");
	RTK_LOGI(TAG, "=========================================\n");

	RTK_LOGI(TAG, " SDSR : %08x\n", IPSEC->SDSR);
	RTK_LOGI(TAG, " SDFW : %08x\n", IPSEC->SDFW);
	RTK_LOGI(TAG, " SDSW : %08x\n",  IPSEC->SDSW);
	RTK_LOGI(TAG, " RST_ISR_CON : %08x\n",  IPSEC->RST_ISR_CON);
	RTK_LOGI(TAG, " IMR : %08x\n", IPSEC->IMR);
	RTK_LOGI(TAG, " DEBUG : %08x\n", IPSEC->DEBUG);
	RTK_LOGI(TAG, " ERR_STAT : %08x\n",  IPSEC->ERR_STAT);
	RTK_LOGI(TAG, " A2EO_SUM : %08x\n",  IPSEC->A2EO_SUM);
	RTK_LOGI(TAG, " ENL_SUM : %08x\n", IPSEC->ENL_SUM);
	RTK_LOGI(TAG, " APL_SUM : %08x\n", IPSEC->APL_SUM);
	RTK_LOGI(TAG, " EPL_SUM : %08x\n",  IPSEC->EPL_SUM);
	RTK_LOGI(TAG, " SWAP_BURST : %08x\n",  IPSEC->SWAP_BURST);
	RTK_LOGI(TAG, " ENG_DBG : %08x\n", IPSEC->ENG_DBG);
	RTK_LOGI(TAG, " DMA_LX_DBG : %08x\n", IPSEC->DMA_LX_DBG);
	RTK_LOGI(TAG, " DMA_RX_DBG : %08x\n",  IPSEC->DMA_RX_DBG);
	RTK_LOGI(TAG, " DMA_TX_DBG : %08x\n",  IPSEC->DMA_TX_DBG);
	RTK_LOGI(TAG, " STA_SDES_CFG : %08x\n", IPSEC->STA_SDES_CFG);
	RTK_LOGI(TAG, " STA_SDES_PTR : %08x\n", IPSEC->STA_SDES_PTR);
	RTK_LOGI(TAG, " STA_SDES_CMD1 : %08x\n",  IPSEC->STA_SDES_CMD1);
	RTK_LOGI(TAG, " STA_SDES_CMD2 : %08x\n",  IPSEC->STA_SDES_CMD2);
	RTK_LOGI(TAG, " STA_SDES_CMD3 : %08x\n",  IPSEC->STA_SDES_CMD3);
	RTK_LOGI(TAG, " STA_DDES_CFG : %08x\n",  IPSEC->STA_DDES_CFG);
	RTK_LOGI(TAG, " STA_DDES_PTR : %08x\n", IPSEC->STA_DDES_PTR);
	RTK_LOGI(TAG, " DBG_VERSION : %08x\n", IPSEC->DBG_VERSION);
	RTK_LOGI(TAG, " DDSR : %08x\n",  IPSEC->DDSR);
	RTK_LOGI(TAG, " DDFW : %08x\n",  IPSEC->DDFW);
	RTK_LOGI(TAG, " DDSW : %08x\n", IPSEC->DDSW);
	RTK_LOGI(TAG, " DESC_PKT_CONF : %08x\n", IPSEC->DESC_PKT_CONF);
	RTK_LOGI(TAG, " DBG_SD : %08x\n",  IPSEC->DBG_SD);
	RTK_LOGI(TAG, " DBG_DD : %08x\n",  IPSEC->DBG_DD);
}
...
void CRYPTO_SetSrcDesc(u32 sd1, u32 sdpr)
{
	...
	while (1) {
		if ((IPSEC->SDSR & SFIFO_EMPTY_CNT) > 0) {
			RTK_LOGI(TAG, "sd1=0x%x , sdpr=0x%x \r\n", sd1, sdpr);
			IPSEC->SDFW = sd1;
			IPSEC->SDSW = sdpr;
			break;
		}
		timeout--;
		if (timeout == 0) {
			RTK_LOGE(TAG, "Timeout, src fifo is FULL \r\n");
			break;
		}
	}
}
...
void CRYPTO_SetDstDesc(u32 dd1, u32 ddpr)
{
	...
	if ((IPSEC->DDSR & DFIFO_EMPTY_CNT) > 0) {
		RTK_LOGI(TAG, "dd1=0x%x , ddpr=0x%x \r\n", dd1, ddpr);
		IPSEC->DDFW = dd1;
		IPSEC->DDSW = ddpr;
	} else {
		RTK_LOGE(TAG, "dst fifo_cnt is not correct: %d \r\n", (IPSEC->DDSR & DFIFO_EMPTY_CNT));
	}
}
	...
	if (auth_type != AUTH_TYPE_NO_AUTH) {
		...
		if (pIE->isSHA2) {
			...
			case AUTH_TYPE_SHA2_256 :
				...
				break;
			default :
				RTK_LOGE(TAG, "No this auth_type(%d) for SHA2\n", auth_type);
					 ...
					 OUT u8 *pResult, OUT u8 *pTag)
{
	...
	while (1) {
		if (IPSEC->RST_ISR_CON & CMD_OK) {
			break;
		}

		ips_err = IPSEC->ERR_STAT;
		if (ips_err) {
			RTK_LOGE(TAG, "ips 0x1C err = 0x%08x\r\n", ips_err);
			return FAIL;
		}

		loopWait--;
		if (loopWait == 0) {
			RTK_LOGE(TAG, "Wait Timeout\r\n");
