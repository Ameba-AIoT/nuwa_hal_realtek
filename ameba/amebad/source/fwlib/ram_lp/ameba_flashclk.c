...
static const char *const TAG = "FLASHCLK";
...
void flash_calibration_backup(u8 flash_speed, u8 read_mode)
{
	...
	RRAM->FLASH_quad_valid_cmd = flash_init_para.FLASH_quad_valid_cmd;
	RRAM->FLASH_dual_valid_cmd = flash_init_para.FLASH_dual_valid_cmd;
	...
	RRAM->FLASH_rd_dummy_cycle0 = flash_init_para.FLASH_rd_dummy_cycle[0];
	RRAM->FLASH_rd_dummy_cycle1 = flash_init_para.FLASH_rd_dummy_cycle[1];
	RRAM->FLASH_rd_dummy_cycle2 = flash_init_para.FLASH_rd_dummy_cycle[2];
}
...
int flash_calibration_highspeed(u8 div)
{
	...
	if (_flash_calibration_highspeed(spic_mode, div) == TRUE) {
		...
		RTK_LOGI(TAG, "FLASH CALIB OK, [%d, %d]\n", flash_init_para.FLASH_rd_sample_phase_cal, flash_init_para.phase_shift_idx);
	} else {
		/* calibration fail, revert SPIC clock to XTAL */
		RCC_PeriphClockSource_SPIC(BIT_SHIFT_FLASH_CLK_XTAL);
		RTK_LOGE(TAG, "FLASH CALIB[NEW FAIL]\n");
		Ret = FAIL;
	}
...
int flash_rx_mode_switch(u8 read_mode)
{
	...
	for (i = read_mode; i < 5; i++) {
		if (i == ReadQuadIOMode) {
			...
			flash_init_para.FLASH_quad_valid_cmd = (BIT_WR_BLOCKING | BIT_RD_QUAD_IO | BIT_PRM_EN);

			tmp_dc = flash_init_para.FLASH_rd_dummy_cycle[2];
			flash_init_para.FLASH_rd_dummy_cycle[2] -= QUAD_PRM_CYCLE_NUM;
			...
			flash_init_para.FLASH_quad_valid_cmd = (BIT_WR_BLOCKING | BIT_RD_QUAD_O);

			flash_init_para.FLASH_rd_dummy_cycle[2] = FLASH_DM_CYCLE_4O;
			tmp_dc = flash_init_para.FLASH_rd_dummy_cycle[2];
			...
			flash_init_para.FLASH_pseudo_prm_en = 1;
			flash_init_para.FLASH_dual_valid_cmd = (BIT_WR_BLOCKING | BIT_FRD_SINGEL  | BIT_CTRLR0_CH | BIT_PRM_EN);

			tmp_dc = flash_init_para.FLASH_rd_dummy_cycle[1];
			flash_init_para.FLASH_rd_dummy_cycle[1] -= DUAL_PRM_CYCLE_NUM;
			...
			flash_init_para.FLASH_pseudo_prm_en = 0;
			flash_init_para.FLASH_dual_valid_cmd = (BIT_WR_BLOCKING | BIT_RD_DUAL_I);

			flash_init_para.FLASH_rd_dummy_cycle[1] = FLASH_DM_CYCLE_2O;
			tmp_dc = flash_init_para.FLASH_rd_dummy_cycle[1];
			...
			tmp_dc = flash_init_para.FLASH_rd_dummy_cycle[0];
		...
		if (_memcmp(pdata, SPIC_CALIB_PATTERN, 8) == 0) {
			RTK_LOGI(TAG, "read_mode:%d\n", i);
			break;
		} else {
			if (flash_init_para.debug) {
				RTK_LOGE(TAG, "read mode %d fail\n", i);
			}
		}
	}

	if (i == 5) {
		RTK_LOGE(TAG, "Flash read mode switch FAIL!\n");
		Ret = FAIL;
	}
