...
u32 SOCPS_SYSIrq(void *Data)
{
	...
	if (tickless_debug) {
		//RTK_LOGI(TAG, "SOCPS_SYSIrq %x Aon Wake_Event %x\n", WakeEvent, WakeEventAon);
		if (WakeEventFlag == FALSE) {
			RTK_LOGS(NOTAG, RTK_LOG_ERROR, "Oops: SOCPS_SYSIrq Error %x !!!!!\n", WakeEvent);
		}
	}
	WakeEventFlag = FALSE;
	return 0;
...
void SOCPS_WakeFromPG(void)
{
	...
	if (ps_config.km0_tickles_debug) {
		RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "SOCPS_WakeFromPG \n");
	}
...
void SOCPS_TimingTest(void)
{
	...
	RTK_LOGI(TAG, "%d(%d) ", WAKE2RFON, WAKE2RFON_AVER);
	RTK_LOGI(TAG, "%d(%d) ", RFON2RFOFF, RFON2RFOFF_AVER);
	RTK_LOGI(TAG, "%d(%d) ", RFOFF2CG, RFOFF2CG_AVER);
	//RTK_LOGI(TAG, "%d ", RFOFF2WIFI32K);
	RTK_LOGI(TAG, "%d(%d) ", WIFI32K2CG, WIFI32K2CG_AVER);
	//RTK_LOGI(TAG, "%d ",ps_config.km0_dur1*30);//WAKE->RFON
	RTK_LOGI(TAG, "\n");
...
void SOCPS_SleepCG(void)
{
	...
	if (nDeviceIdOffset != PMU_MAX) {
		...
		RTK_LOGS(NOTAG, RTK_LOG_ERROR, "Oops: Sleep Fail %x !!!!!\n", nDeviceIdOffset);
		SOCPS_SleepDeInit();
		return;
	}

	if (ps_config.km0_tickles_debug) {
		RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "LPCG\n");
	}
	...
	if (ps_config.km0_tickles_debug) {
		RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "LPCW\n");
	}
...
void SOCPS_SleepPG(void)
{
	...
	if (ps_config.km0_tickles_debug) {
		RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "LPPG\n");
	}
	...
	//RTK_LOGI(TAG, "SOCPS_SleepPG wakeup \n");
	...
	if (ps_config.km0_tickles_debug) {
		RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "LPPW\n");
	}
