...
SRAM_NOCACHE_DATA_SECTION rtos_task_t log_buffer_thread_handle;

#if defined (CONFIG_ARM_CORE_CM4)
static const char *const TAG = "RTLTRACE";
...
u32 LOG_PRINTF_BUFFER_INIT(u32 thread_init)
{
	...
	if (log_buffer_start == 0) {
		RTK_LOGI(TAG, "KM0 dont open logbuf \n");
		return 0;
	}
	...
	RTK_LOGI(TAG, "LOG_PRINTF_BUFFER_INIT KM4 %x %x\n", log_buffer_start, log_buffer_end);
...
u32 LOG_BUFF_SUSPEND(void)
{
	if (log_buffer_thread_handle) {
		rtos_task_suspend(log_buffer_thread_handle);
	}
	return TRUE;
}
...
{
	if (log_buffer_thread_handle) {
		rtos_task_resume(log_buffer_thread_handle);
	}
	return TRUE;
}
#endif
