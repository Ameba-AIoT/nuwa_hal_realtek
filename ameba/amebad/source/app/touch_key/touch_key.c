...
static const char *const TAG = "KEYSCAN";
...
void app_hp_jack_init(void)
{
	...
	if (xTimers == NULL) {
		RTK_LOGE(TAG, "HP timer create error\n");
	}
#endif
...
void app_captouch_init(void)
{
	...
	//RTK_LOGI(TAG, "km4 app_captouch_init dslp_wakeup:%x \n", dslp_wakeup);
}
...
static void app_keyscan_data_process(u32 ksdata)
{
	...
	RTK_LOGI(TAG, "ksdata:%x (row:%x col:%x press:%x) \n", ksdata, row, col, press);
}
...
void app_keyscan_init(u8 reset_status)
{
	...
	//RTK_LOGI(TAG, "app_keyscan_init column_sel:%08x, row_sel:%08x\n", column_sel, row_sel);
